        ...
        try {
            ...
            if(dataFindProject){
                ...
                if(diff < 24) {
                    ...
                    const { data: dataCreateSearchLog, error: errorCreateSearchLog } = await app.src.services.repositories.create('searchlog', { user: user, project: dataFindProject.repository })

                    if(errorCreateSearchLog) {
                        logger.error({message: "Error saving on Database", meta: errorCreateSearchLog})
                ...
                if(errorUpdateProject){
                    logger.error({message: "Error updating on Database", meta: errorUpdateProject})
                    return replies.unprocessableEntity(response)('Error.')
                }
            } 
            ...
            logger.info(`GET https://api.github.com/search/issues?q=repo:${repo}+state:open&sort=created&order=desc&per_page=100&page=1`)
            let result = await getResourceAsync()

            if(!result || !result._linkHeaders) {
                logger.error({message: "Error requesting to GitHub API", meta: new Error(result)})
                return replies.conflict(response)('Error requesting to GitHub API')
            } 
            ...
            if(errorCreateProjectInfo){
                logger.error({message: "Error saving on Database", meta: errorCreateProjectInfo})
                return replies.unprocessableEntity(response)('Error.')
            } 

            // So we create the log from this search using the User and Project/Repository and save at Searchlog table
            const { data: dataCreateSearchLog, error: errorCreateSearchLog } = await app.src.services.repositories.create('searchlog', { user: user, project: repo })
